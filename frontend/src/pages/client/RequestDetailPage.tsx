...
export default function RequestDetailPage() {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      // Em um cenário real, chamaríamos a API
      // await axios.post(`http://localhost:3000/api/client/requests/${id}/messages`, { content: newMessage })
  ...
  const handleCancelRequest = async () => {
    ...
    try {
      // Em um cenário real, chamaríamos a API
      // await axios.patch(`http://localhost:3000/api/client/requests/${id}`, { status: 'CANCELLED' })
      
      // Para o MVP, simulamos o cancelamento
      await new Promise(resolve => setTimeout(resolve, 500))
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        ...
        <div className="p-6 border-b border-gray-200">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <div className="space-y-2">
                ...
                <div>
                  ...
                  <span className="ml-2">{request.serviceDetails?.category || 'Não especificada'}</span>
                </div>
            ...
            <div>
              <h3 className="text-lg font-semibold mb-3">Informações do Prestador</h3>
              
              <div className="space-y-2">
                <div>
                  <span className="text-gray-500">Nome:</span>
                  <Link to={`/providers/${request.providerId}`} className="ml-2 text-primary-600 hover:text-primary-700">
              ...
              >
                <div className="flex justify-between items-start mb-1">
                  <span className="font-medium">
                    {message.sender === 'CLIENT' ? 'Você' : request.providerName}
                  </span>
